...
OUTPUT_NAME_PATTERN = [
    ...
    "[_space-{space}][_cohort-{cohort}][_desc-{desc}]_{suffix}"
]
...
def corresponding_file(in_file, bids_dir, extension, bids_validate=True):
    ...
    corresponding_filename = layout.build_path(entities, OUTPUT_NAME_PATTERN, validate=False)

    corresponding_filename = Path(corresponding_filename)
    if not corresponding_filename.is_absolute():
        corresponding_filename = in_file.parent / corresponding_filename
